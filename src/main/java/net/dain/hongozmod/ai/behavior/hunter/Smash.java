package net.dain.hongozmod.ai.behavior.hunter;
...
/*
public class Smash extends Behavior<HunterEntity> {
    ...
    public Smash() {
        super(ImmutableMap.of(MemoryModuleType.ATTACK_TARGET, MemoryStatus.VALUE_PRESENT, MemoryModuleType.SMASH_COOLDOWN, MemoryStatus.VALUE_ABSENT, MemoryModuleType.SMASH_SOUND_COOLDOWN, MemoryStatus.REGISTERED, MemoryModuleType.SMASH_SOUND_DELAY, MemoryStatus.REGISTERED), DURATION);
    }

    protected boolean checkExtraStartConditions(ServerLevel pLevel, HunterEntity pOwner) {
        return pOwner.closerThan(pOwner.getBrain().getMemory(MemoryModuleType.ATTACK_TARGET).get(), 15.0D, 20.0D);
    }

    protected boolean canStillUse(ServerLevel pLevel, HunterEntity pEntity, long pGameTime) {
        return true;
    }

    protected void start(ServerLevel pLevel, HunterEntity pEntity, long pGameTime) {
    ...
    protected void tick(ServerLevel pLevel, HunterEntity pOwner, long pGameTime) {
    ...
    protected void stop(ServerLevel pLevel, HunterEntity pEntity, long pGameTime) {
